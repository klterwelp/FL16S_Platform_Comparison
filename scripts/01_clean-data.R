...
# STEP THREE: CLEAN METADATA AND SAMPLE NAMES
...
# HOMD database taxonomy uses "species" only, add "Genus"
...
# add rank "ASVs"
# set HOMD Species to HOMD_Species
f_replace_blast_species <- function(tax_table) {
  tax_table_ranks <- colnames(tax_table)
  tax_table <- tax_table %>% tax_mutate(HOMD_Species = Species)
  tax_table <- tax_names2rank(tax_table, colname = "ASV")
  tax_table_asvs <- rownames(tax_table)
...
# STEP FIVE: UPDATE OTU TABLES AND SAVE RDS
...
# STEP SIX: EXPORT LAST IDENTIFIED TAXON TO TSV
if (!dir.exists(out_tsv)) {
  dir.create(out_tsv)
}
ranks <- rank_names(v1v3_ps) # extract taxonomic ranks
# function to get the last identified taxa and its rank from one row
get_last_ided <- function(taxon_row) {
  tax_levels <- taxon_row[-1]  # excludes ASV column
...
# function to write all last identified taxons to a TSV
f_write_last_tax <- function(tax_table, out_file) {
  tax_df <- as.data.frame(tax_table, stringsAsFactors = FALSE)
  # going to remove anything after _ 
  tax_df <- tax_df %>%
    select(-HOMD_Species) %>%
    mutate(across(where(is.character), ~ sub("_.*", "", .))) %>%
    relocate(ASV) %>%
    rowwise() %>%
    mutate(last_info = list(get_last_ided(c_across(everything())))) %>%
    transmute(
      ASV = ASV,

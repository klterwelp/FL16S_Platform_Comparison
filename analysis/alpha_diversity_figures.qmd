...
median_alphas <- rarefied_alpha %>% 
  select(shrt_names, med_evenness,med_richness, med_shannon, shrt_sample_dna, total_reads, analysis) %>% 
  distinct()  %>% 
  mutate(analysis = factor(analysis, levels = c("MGX", "FL-16S", "V1-V3")))


median_alphas_long <- pivot_longer(rarefied_alpha, 
...
median_alphas_long %>% 
  ...
  add_significance() %>% 
  select(group1, group2, Measurement, estimate, p.adj, p.adj.signif)
```

...
# uses kruskal-wallis to not assume normality 
median_alphas_long_test %>% 
...
median_alphas_long %>% 
  group_by(Measurement) %>% 
  pairwise_wilcox_test(formula = value ~ analysis, detailed = TRUE) %>% 
  add_significance()
```
...
f_export_plot(plot = last_plot(), out_file = "alpha_diversity_metrics_1000-bootstrap-median.png")
```
